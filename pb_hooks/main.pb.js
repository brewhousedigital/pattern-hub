...
onRecordAfterUpdateSuccess((e) => {
  ...
  //
  // Not fired from onRecordAfterCreateSuccess: opengraph_image is generated by
  // a second, separate PocketBase update a few seconds after the initial create
  // (AdminEditPatternModal.tsx - it needs the record's own file URL to exist
  // first, so it can't happen in the same request). Firing on create would
  // always post before the image exists. Instead:
  //   - the "brand new + immediately published" case is triggered explicitly by
  //     the frontend via /api/notify-discord-pattern, right after that second
  //     update lands (so the embed always has its image);
  //   - the "drafted earlier, published later via a plain edit" case is caught
  //     below by the create->publish transition (whatever image already exists
  //     by then, since no file is being touched in that save).
