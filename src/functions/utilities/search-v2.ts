...
export const SORT_OPTIONS = [
  ...
  { value: '-done_count', label: 'Most completed' },
  { value: 'done_count', label: 'Least completed' },
  { value: 'no_ratings', label: 'No ratings' },
  { value: 'no_difficulty_ratings', label: 'No difficulty ratings' },
  { value: 'unloved', label: 'Unloved' },
...
// ─────────────────────────────────────────────────────────────────────────────
//
// Token → filter translation (both the PocketBase filter-DSL string and the
// raw-SQL WHERE fragment for facet counting) now lives server-side in
// pb_hooks/main.pb.js's buildPatternFilters, next to /api/pattern-search - see
// the comment there. Keeping one implementation instead of a client/server
// pair removes the risk of the two drifting apart. This file still owns the
// Token type and the URL ⇄ token conversions above, which the client sends
// to that endpoint as-is.
